...
class LightingManager:
    ...
    def init_light_sources(self):
        ...
        if self.game.world and self.game.world.map_manager:
            ...
            # [수정] 맵 캐시가 (x,y,z) 형태이므로 z=0인 경우만 광원으로 추가
            for tid in target_ids:
                if tid in tile_cache:
                    for px, py, pz in tile_cache[tid]: # (x,y,z)를 받음
                        if pz == 0: # 1층 광원만 추가 (혹은 필요한 z레벨만)
    ...
    def apply_lighting(self, camera):
        ...
        if final_alpha > 50:
            ...
            for lx, ly in self.light_sources:
                # [수정] 광원도 Z-level 고려 (현재는 0층 광원만 사용)
                # if player.z_level == 0: # 플레이어가 0층에 있을 때만 0층 광원 적용 등 복잡한 로직 추가 가능
            ...
            if not (self.game.current_phase == 'DAWN' and player.role != "MAFIA"):
                ...
                poly_points_abs = self.game.fov.get_poly_points(player.rect.centerx, player.rect.centery, radius_tiles, direction, 60, z_level=player.z_level)
