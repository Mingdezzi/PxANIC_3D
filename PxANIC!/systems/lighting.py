...
class LightingManager:
    ...
    def get_shadow_params(self):
        ...
        # Return shadow shift factors (dx, dy) per unit height
        
        phase = self.game.current_phase
        durations = self.game.game.shared_data.get('custom_durations', DEFAULT_PHASE_DURATIONS)
        total_time = durations.get(phase, 60)
        progress = 1.0 - (self.game.state_timer / max(total_time, 1))
        progress = max(0.0, min(1.0, progress))

        # Sun Position Simulation
        # X: -1 (Left/East) to 1 (Right/West)
        # Y: -0.5 (North-ish, so shadows cast South/Down)
        # Z: Height (0 at Horizon, 1 at Zenith)
        
        sun_x = 0.0
        sun_z = 0.5 # Default
        
        if phase == 'DAWN':
            sun_x = -1.0 + (progress * 0.3) # -1.0 -> -0.7
            sun_z = 0.1 + (progress * 0.4)  # 0.1 -> 0.5
        elif phase == 'MORNING':
            sun_x = -0.7 + (progress * 0.5) # -0.7 -> -0.2
            sun_z = 0.5 + (progress * 0.4)  # 0.5 -> 0.9
        elif phase == 'NOON':
            sun_x = -0.2 + (progress * 0.4) # -0.2 -> 0.2
            sun_z = 0.9 # Keep High
        elif phase == 'AFTERNOON':
            sun_x = 0.2 + (progress * 0.5)  # 0.2 -> 0.7
            sun_z = 0.9 - (progress * 0.4)  # 0.9 -> 0.5
        elif phase == 'EVENING':
            sun_x = 0.7 + (progress * 0.3)  # 0.7 -> 1.0
            sun_z = 0.5 - (progress * 0.4)  # 0.5 -> 0.1
        elif phase == 'NIGHT':
            # Moon Cycle
            sun_x = -0.8 + (progress * 1.6) # -0.8 -> 0.8
            sun_z = 0.4 
            
        sun_y = -0.4 # Constant "North" sun to cast "South" (Down) shadows
        
        # Calculate Shift (Shadow Vector per Unit Height)
        # Clamp Z to avoid infinite shadows
        safe_z = max(0.2, sun_z)
        
        # Shift = -LightPos / LightHeight
        shift_x = -(sun_x / safe_z)
        shift_y = -(sun_y / safe_z)
        
        # Limit max length
        max_shift = 2.5
        dist = math.hypot(shift_x, shift_y)
        if dist > max_shift:
